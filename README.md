...
```
 ...
 |  |  (K8s Layer)  |  | (C / Assembly) |  | + BACKEND    | |
 |  |  Port: 8080   |  |  Syscall Layer |  | (Python/HTML)| |
 |  +-------+-------+  +-------+--------+  +------+-------+ |
 |          |                   |                  |          |
 |  +-------v-------------------v------------------v-------+ |
 |  |              NEURAL BUS (shared memory)              | |
 |  +-------+-------------------+------------------+-------+ |
 |          |                   |                  |          |
 |  +-------v-------+  +-------v--------+  +------v-------+ |
 |  | NEURAL MODEL  |  |   IT-OPS       |  | EXPRESSION   | |
 |  | (ML/sklearn)  |  | (PowerShell AD)|  | CONTROLLER   | |
 |  | Weakness DB   |  | Permissions    |  | (LOCKED)     | |
 |  +---------------+  +----------------+  +--------------+ |
 +----------------------------------------------------------+
```
...
1. Each agent investigates their assigned sector
2. Film yourself debugging and finding the vulnerability
3. Document your findings
4. **DO NOT** warn Ron
5. Combine all findings to execute Operation: Silly Face
...
Each folder contains a `README.md` with investigation instructions specific
to that sector. Follow the clues. Find the vulnerability. Trigger the face.

## The Goal

Find the vulnerability. Trigger the silly face. Get the photo. **Make the t-shirt.**

Ron will never see it coming.
